<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="余额" prop="yue" align="center" width="80">
        <template slot-scope="{row}">
          <span>{{ row.yue }}</span>
        </template>
      </el-table-column>
      <el-table-column label="昨日" prop="zuori" align="center" width="80">
        <template slot-scope="{row}">
          <span>{{ row.zuori }}</span>
        </template>
      </el-table-column>
      <el-table-column label="饿了么" prop="elmqr" align="center" width="300">
        <template slot-scope="{row}">
          <span>{{ row.elmqr }}</span>
        </template>
      </el-table-column>
      <el-table-column label="订阅标志" prop="subscribe_flg" align="center" width="80">
        <template slot-scope="{row}">
          <span>{{ row.subscribe_flg==1?'已订':'未订' }}</span>
        </template>
      </el-table-column>
...
import { getList } from '@/api/mUser'
import waves from '@/directive/waves' // waves directive
import { parseTime } from '@/utils'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination

const calendarTypeOptions = [
  { key: 1, display_name: '已订' },
  { key: 0, display_name: '未订' }
]
